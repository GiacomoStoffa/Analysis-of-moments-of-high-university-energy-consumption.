...
# SUPER SOM INIT ---------------------------------------------------------------
som.grid = somgrid(xdim=5, ydim=5, topo = "hexagonal", toroidal = 1)
...
# kmeans
ss.km = kmeans(as.matrix(object.distances(ss, "codes")), n_cluster)[["cluster"]]
...
# Mapping
plot(ss, type = "mapping", bgcol=bgcolors[as.integer(ss.km)], pch = 1, main = "Mapping", keepMargins = TRUE)
# Layers
plot(ss, bgcol=bgcolors[as.integer(ss.km)])
# Distances
